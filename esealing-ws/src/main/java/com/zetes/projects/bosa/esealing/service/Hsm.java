...
class Hsm {
	...
	// Get the trusted cert from the keystore, if there are more then return the one with the specifry serialNumber or otherwise the last one found.
		...
		try {
			...
			while (aliases.hasMoreElements()) {
				...
				if (ks.isCertificateEntry(alias)) {
					ret = (X509Certificate) ks.getCertificate(alias);
